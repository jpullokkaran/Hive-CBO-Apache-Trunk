...
public class HiveTableScanRel extends TableAccessRelBase implements HiveRel {
  ...
  public HiveTableScanRel(RelOptCluster cluster, RelTraitSet traitSet,
    ...
    m_hiveColStat.addAll(OptiqStatsUtil.computeTableRelColStat(table, m_colNamesLst));
...
/*
  @Override
    public long getColumnAvgSize(List<Integer> projIndxLst) {

        if (projIndxLst != null) {
            List<String> projLst = new LinkedList<String>();

            for (Integer projIdx : projIndxLst) {
                projLst.add(m_projIdToNameMap.get(projIdx));
            }

            if (!m_colNamesLst.equals(projLst))
                return ((RelOptHiveTable) table).getColumnStats(projLst)
                        .getAvgRowSize();
        }
